...
export function LaunchPreviewStep({
  slug,
  lifecycleStatus,
  readinessInput,
  locale = 'en',
}: LaunchPreviewStepProps) {
  const gate = resolveGuestPathGate(readinessInput);
  const trimmedSlug = slug.trim();
  const landingUrl = trimmedSlug ? getTenantPublicUrl(trimmedSlug, 'landing', locale) : null;
  const appUrl = trimmedSlug ? getTenantPublicUrl(trimmedSlug, 'app', locale) : null;
    ...
    <div className="space-y-5">
      ...
      {trimmedSlug ? (
        <div className="space-y-2 rounded-xl border p-4">
          ...
          <p className="text-[11px] text-muted-foreground">Links open preview in a new tab.</p>
        </div>
      ) : (
